...
k_sum=100
n=2^10
x_sparse=sparse.random(n, 1, density=k_sum/n, data_rvs=np.ones)
incident_vector=x_sparse.toarray()
stage_index=4
size_of_group=n/2**stage_index
k=[]
for i in range(n/2**4):
    num_of_defectives=np.sum(incident_vector[i*size_of_group:(i+1)*size_of_group])
    if num_of_defectives>0:
        k.append(num_of_defectives)
